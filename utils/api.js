export async function generateReport(images) {
  ...
  console.log('Sending FormData to http://10.99.20.184:5000/api/analyze...', {
  ...
  try {
    const response = await fetch('http://10.99.20.184:5000/api/analyze', {
      method: 'POST',
      body: formData,
    });
...
export async function getChatResponse(userMessage, { systemPrompt, context, conversationHistory }) {
  console.log('Sending chat request to http://10.99.20.184:8081/api/chat...', {
  ...
  try {
    const response = await fetch('http://10.99.20.184:5000/api/chat', {
      method: 'POST',
